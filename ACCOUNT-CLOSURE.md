...
- **~~H11~~ — RESOLVED BY THE RENAME.** The finding asked the admin path to make `username` clearable on a closed account, so a closure that failed halfway could be finished by hand. The rename answered it instead: `close()` writes `username` **and** `usernameLower` onto `closed-<last 10 of id>` inside the same `tx.user.update` that stamps `accountClosedAt`, so the handle is released by the closure write itself — a row carrying that stamp has already handed it back, and a row without one was never closed. The clear is also no longer a clear: `username` is non-null now, so it was a constraint violation rather than a cleared field, and unreachable besides (`UpdateUserDto` refuses `''` with `@MinLength(3)`). `admin.service.ts` therefore refuses an empty username on **any** account, and what it took from this finding instead is the pairing rule — every admin rename writes `usernameLower` with it, or the unique index, sign-up's availability check and login's identifier match all keep pointing at the old handle. `close-account.service.spec.ts` locks both halves.
...
7. Admin: a **Close account** action distinct from Ban; an `accountClosedAt != null` chip in the user list that is **not** the red BANNED chip; and a `'closed'` filter. ~~Make `username` clearable at `admin.service.ts:501` (H11).~~ **Dropped — see H11:** the closure rename releases the handle on its own, so the admin path keeps refusing an empty username and carries `usernameLower` on every rename instead.
