...
| [`docs/design/licence-centre/SPEC-BUILD.md`](design/licence-centre/SPEC-BUILD.md) | You are building the **new `/licence-centre` review sheet** (Motivation Centre rebuild, Phase 3 frontend). Written 2026-09-08 from the design canvas: artboard → route map, the tokens and type scale it matched, the one `sheet-row.tsx` component and its four states, the shelf, the strip's single missing-count, behaviour rules, traps, and the build order with a check per step. The `*.dc.html` files beside it are the pixel spec; `MOTIVATION-REBUILD-BRIEF.md` at the repo root is the scope. |
