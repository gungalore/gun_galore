...
import { CrimeStatsModule } from './crime-stats.module';
...
describe('CrimeStatsModule', () => {
  it('boots with its controllers and guards resolved', async () => {
    const mod = await Test.createTestingModule({
      imports: [StubGlobalsModule, CrimeStatsModule],
    })
      .overrideProvider(PrismaService)
      .useValue({})
      .compile();

    expect(mod.get(CrimeStatsService)).toBeInstanceOf(CrimeStatsService);
    expect(mod.get(CrimeStatsFetchService)).toBeInstanceOf(CrimeStatsFetchService);
    await mod.close();
  });
});
