...
describe('the things that strengthen it', () => {
  it('are listed but never counted as missing requirements', () => {
    // A safe photograph is what makes a motivation land; its absence is not a
    // reason SAPS turns someone away.
    const s = documentStatus(S13, []);
    const safe = s.needs.find((n) => n.kind === K.SAFE_PHOTO)!;
    expect(safe.tier).toBe('strengthens');
    expect(s.missingRequired).not.toContain(K.SAFE_PHOTO);
  });

  it('explain themselves in the applicant\'s terms', () => {
    const s = documentStatus(S13, []);
    const safe = s.needs.find((n) => n.kind === K.SAFE_PHOTO)!;
    expect(safe.why).toMatch(/three shots/i);
