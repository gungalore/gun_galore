...
6. Plain, sober South African English. No Americanisms. The words that
   ...
   them.
   ⚠️ AND THE PRODUCT-PAGE WORDS ARE REFUSED ANYWHERE IN THE DOCUMENT,
   not only where the firearm is described. ONE of them, ONCE, and the
   document is not filed at all and the applicant is told we could not finish
   it. Always refused: "platform", "tactical", "engage targets", "dynamic",
   "competitively", "efficiently", "power factor", "split times", "high
   volume", "magazine capacity", "high capacity", "polymer frame",
   "striker-fired", "short-recoil", "tilting barrel", "safe action",
   "ergonomic", "reliability under", "maintenance cycle", "proven track
   record", "battle-proven", "combat-proven", "state-of-the-art",
   "cutting-edge". The ballistics words are NOT in this list because the
   comparison rule governs them instead and says exactly where they may
   appear.
   Ordinary words carry the same meaning and are accepted. A rifle is a
   rifle. A shooter SHOOTS AT targets, or fires a course of fire. A
   discipline is fast, close-range or unsupported rather than "dynamic". A
...
const SECTION_BRIEFS: Record<SectionId, string> = {
   ...
   *
   * It used to quote one in full, naming a real make and model: "built on
   * the <make> <model> platform, utilising a turn-bolt, push-feed repeating
   * action housed in a robust one-piece forged steel receiver".
   *
   * MO000075 then came back having rewritten that sentence almost word for
   * word - same clause order, same unusual pairings, the same receiver - on
   * an application for the very rifle the example named. It tripped
   * documentScope on "platform" twice running, the whole document was
   * binned and the applicant was emailed to say we could not finish it. The
   * rest of that draft was clean: structureOk true, sameness 0.00.
   *
   * A WELL-FORMED NEGATIVE EXAMPLE IS A TEMPLATE. The model matched the
   * pattern and read straight past the prohibition wrapped around it - and
   * it was likeliest to do so exactly when the example named the applicant's
   * own firearm, which is the case the example existed for.
   *
   * So: name the fault, never hand over a usable sentence, and never put a
   * real make or model in an example of what not to write. The list of
   * refused WORDS stays - a word list cannot be copied into a sentence the
   * way a sentence can.
   */
  the_firearm:
