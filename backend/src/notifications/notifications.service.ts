...
export class NotificationsService {
  ...
  async newSaleSeller(d: SaleDetails) {
    ...
    // the seller hit Accept with one tap, no the identity provider sign-in required.
...
<p style="margin: 8px 0; font-size: 13px; color: #666;">Pre-filled fields are a convenience only — please double-check every value against your licence before signing. Sections for the police and the dealer have been left blank on purpose.</p>`,
  ...
  async offerReceived(d: {
     ...
     * needing an identity provider sign-in. Falls back to the dashboard URL when
     * absent.
     */
    actionUrl?: string;
  }) {
    ...
    // phone with no the identity provider session). The character budget is tight —
  ...
  async offerAccepted(d: {
     ...
     * /checkout/offer/[id] URL (which requires the identity provider auth).
     */
    actionUrl?: string;
  }) {
