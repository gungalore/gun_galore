...
- **Standalone PAID Ballistic Calculator PWA — `ballistics.gungalore.co.za`
  (R299 lifetime, mostly shipped).** Separate product on its own
  subdomain, same Next.js codebase + shared Clerk/Prisma/Peach. Host-
  based middleware rewrites the subdomain root to `/ballistics`. The
  killer differentiator: AI Bullet Lookup — type "175gr SMK" and
  Claude (Haiku 4.5, ~$0.0003/call) returns weight + BC G1/G7 + typical
  MV + sources. New schema fields on `User`: `ballisticsPurchasedAt`,
  `ballisticsPurchasePeachId`, `ballisticsPurchaseAmountCents`,
  `ballisticsDemoLookupAt`. New models: `UserBallisticProfile` (20-
  profile cap, sync via `/api/ballistics/profiles`) + `ShotMark` (the
  Spot Tracker — GPS+compass+camera+crosshair MARK + recovery arrow,
  wired in BC-9 still to do).
  - `backend/src/ballistics/` now has 5 services:
    `ballistics.service.ts` (the G1 solver),
    `ballistics-bullet-lookup.service.ts` (Claude one-shot),
    `ballistics-purchase.service.ts` (Peach checkout + webhook + admin
    refund, `BAL-<userId>-<nonce>` merchantTxId routes via its own
    `/api/ballistics/webhook/peach` to avoid the marketplace circular
    dep), `ballistics-profiles.service.ts` (CRUD), and the
    `ballistics.controller.ts` (calculate / lookup-bullet / license /
    purchase/checkout / profiles).
  - `ZohoBooksService` gained `createBallisticsLicenseSalesReceipt`
    + `createBallisticsLicenseCreditNote` — deposit to "Bank — Peach
    Pending" → revenue to "Ballistic Calculator Revenue" (or "SaaS
    Revenue" fallback). Operator must create those CoA accounts in
    Books before the first live sale, otherwise the receipt sync
    fails and surfaces in the existing retry-queue.
  - `frontend/lib/ballistics-calc.ts` — full TS port of the backend
    G1 solver + G7 added as second drag model + Haversine forward/
    inverse for Spot Tracker. `backend/src/ballistics/ballistics.
    service.spec.ts` + `backend/scripts/check-ballistics-parity.ts`
    assert backend and frontend agree within ±0.01 mil across .308
    175 SMK, 6.5 CM 140 ELDM, .300 WM 215 Berger fixtures.
    Run `cd backend && npx jest ballistics.service.spec.ts` OR
    `npx ts-node scripts/check-ballistics-parity.ts`.
  - `frontend/app/ballistics/` — scoped instrument palette
    (`--bc-*` tokens under `.bc-app`), Space Grotesk + JetBrains Mono
    via `next/font`. Pages: HuntV1 "Elevation Tank" at `/`,
    `/profile` (host for AILookup), `/purchase` (Peach widget).
    Demo mode (no license) locks the profile to `.308 175 SMK`,
    caps range at 200m, allows exactly 1 lifetime AI lookup, and
    surfaces a persistent "Unlock — R299" CTA.
  - `frontend/lib/ballistics-data/` — bundled offline data files
    (~3-5 KB): 30+ curated SA bullets with G1/G7 BCs, 11 rifle
    presets, 8 SA game-class energy thresholds. Used as offline
    fallback when Claude lookup can't be reached.
  - PWA wired: `/ballistics` carved out of the generic `/offline`
    fallback so the precached shell serves directly (calculator is
    offline-first by design); manifest shortcut added for Android
    long-press menu; "Tools" section in the marketplace MoreSheet
    links to `/ballistics`.
  - **Operator pre-launch checklist (BC-OPS, still to do):**
    1. DNS A record `ballistics.gungalore.co.za` → `139.84.231.220`
       at your registrar
    2. SSH + nginx server block proxying to localhost:3000 + Let's
       Encrypt cert via `certbot --nginx -d ballistics.gungalore.co.za`
    3. Clerk dashboard: set cookieDomain to `.gungalore.co.za` so a
       sign-in on the marketplace also works on the subdomain
    4. Zoho Books CoA: add accounts "Ballistic Calculator Revenue"
       (income) + verify "Bank — Peach Pending" exists (deposit)
  - **Still to ship (BC-7 + BC-9):** Precision mode + DOPE table +
    GPS auto-fill, and the Spot Tracker camera+crosshair+arrow
    flow. Schema is already in place for both.
