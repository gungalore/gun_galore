...
export const BACKEND_ENV_MANIFEST: readonly EnvVar[] = [
  ...
  // ⚠️ NO AWS_* ROWS, AND THEIR ABSENCE IS THE POINT. Four of them sat here
  // (AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY,
  // AWS_KYC_LIVENESS_ROLE_ARN) describing Textract OCR and Rekognition face
  // match / liveness. Nothing under backend/src/ reads an AWS_* name since the
  // Didit cut-over, so keeping them would make Warden report a disabled
  // FEATURE for a capability that no longer exists — on the one board whose
  // job is telling the truth about configuration. They are listed as dead in
  // backend/.env.example under RETIRED instead.
