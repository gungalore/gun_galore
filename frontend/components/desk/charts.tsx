...
/**
 * THE DESK — the charts: Line, BarList, Funnel, Split, and the card they sit in.
 *
 * 🚨 THIS FILE SURVIVED THE DELETION OF THE BOARD IT WAS WRITTEN FOR, AND THE
 * REASON IS WORTH KNOWING BEFORE ANYONE TRIES AGAIN. Pulse was the only
 * PRODUCT surface using these, but app/admin/desk-kit/page.tsx renders
 * LineChart, BarList, Funnel, Split and ChartCard in its Charts and
 * "Funnel & split" panels — it is the fixture the Phase 1 artboard is checked
 * against, it is session-gated, and it is inside desk-guard's own GUARDED
 * list. Deleting charts.tsx is a hard build failure at that import, not a
 * clean removal. What went with Pulse is the pair nothing else rendered:
 * `Heatmap` and `HeatmapCell`, the dow x hour grid. If the kit fixture is ever
 * deleted, this file goes in that commit and not before.
 *
 * ⚠️ AND A SOURCE SCAN FOR THESE NAMES MUST STRIP COMMENTS FIRST. Five files
 * outside the Desk contain the word `Split` — app/(legal)/complaints,
 * app/cart, components/date-field, lib/bench/geometry, lib/scan/detect — and
 * every one of them is the English word inside a comment. A raw grep reports
 * them as consumers.
 *
...
/** A titled chart block, so a board does not re-declare the frame per chart. */
