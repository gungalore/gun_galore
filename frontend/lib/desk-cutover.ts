...
export const CUTOVER_MAP: CutoverRoute[] = [
  ...
  },
  {
    ...
    desk: null,
    coverage: 'none',
    note: 'NEXT: multi-seller orders as a Ledger segment. No design exists; needs one.',
  },
  {
    legacy: '/admin/orders/[id]',
    desk: null,
    coverage: 'none',
    note: 'NEXT: as above, sharing the Order drawer — which now exists and is wired on the Ledger. It reaches a cart parent Order through any one of its lines, so what is missing is a board that lists orders, not the dossier. Same read-only limit as /admin/transactions/[id].',
  },
  {
  ...
  },
  {
    ...
    desk: '/admin/desk/site',
    coverage: 'partial',
    note: 'CORRECTED FROM replaced ON REVIEW, and this is the one on the board that would have hurt most. The roster drawer on Site LISTS administrators and has no control on any row. The legacy page creates an admin (POST /admin/admins), changes a role (PATCH id/role) and deactivates one (POST id/deactivate), each behind the SuperadminGuard. lib/desk-site.ts exports setAdminRole and deactivateAdmin, nothing calls them, and there is no create call at all — while the comment above them claimed the drawer was the only way to grant or revoke admin access. Redirecting this route would have left a database write as the only route to removing a compromised administrator. Wire the three actions, then close it. Role changes also still need the server-side MONITORING_ADMIN guard.',
  },
  {
