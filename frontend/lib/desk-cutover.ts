...
export const CUTOVER_MAP: CutoverRoute[] = [
  ...
  },
  {
    ...
    coverage: 'partial',
    note: "Search carries over in full: the same email / first name / last name / username match, clearable, paged 50 at a time with the footer stating '1–50 of 1,284' rather than a total over a truncated list, and all four legacy quick filters as segments — Closed included, tagged neutral because a closure is a member leaving, not misconduct. Verifying is the broader kyc-outstanding rather than the command centre's >24h kyc-stalled, so everyone stalled is in the segment but the stalled subset cannot be isolated. BULK BAN IS NOW BUILT, TO THE SPEC THIS NOTE LEFT. It was held back rather than skipped: the legacy sweep is safe only because its checkbox column greys out already-banned and closed accounts, and a Desk confirm could not otherwise name a count it could vouch for. The row now carries a DISABLED checkbox — disabled, not hidden, with the reason on hover, because a missing checkbox reads as a rendering glitch — and the confirm names the ELIGIBLE count, lists who is being left alone BY NAME, and refuses over the server's cap of 50. ⚠️ THE SWEEP IS OFF UNTIL TURNED ON: a checkbox column standing open invites a sweep as the normal way to work, and it is not — individual bans belong in the Member drawer where the operator can see who they are banning. ⚠️ AND THE CHECKBOX SITS BESIDE THE ROW BUTTON, NOT INSIDE IT: a checkbox nested in a button is invalid markup browsers resolve by silently dropping one, the same trap the trust-and-safety rows hit. STILL MISSING: the per-row actions menu — edit profile, seller tier, setting kycStatus directly, and close account. Those are writes the Member drawer does not carry either; see /admin/users/[id], where close-account is the shared blocker.",
  },
  {
  ...
  },
  {
    ...
    coverage: 'partial',
    note: "Pulse now reads the same methods over the same period vocabulary, plus a bucket control. 🚨 THE PERIOD GAP WAS A TYPE, NOT A FEATURE — `type Period` stopped at 90d while the comment above it listed all five resolvePeriod() accepts, so 365d and all-time arrived by widening a union. TOP MAKES AND MODELS and TIME TO SALE are now cards, and they were the same story: GET /admin/analytics/top-make-model and /time-to-sale have been serving since the legacy page was written and nothing called them. Makes are ranked by UNITS not rand, because a cheap item that moves often is a different fact from one expensive sale; time to sale is labelled MEDIAN because one listing that sat for a year makes a mean useless, and an unlabelled median gets read as an average and argued with. ⚠️ THE BUCKET REFETCH DEPENDENCY WENT IN WITH THE CHIPS — the loader was keyed on period alone, so the chips would have lit up and fetched nothing. STILL LOST: CSV EXPORT, and it is not a pure frontend job — no analytics CSV endpoint exists (only the unrelated /admin/transactions/export.csv), so it needs a backend route before it needs a button.",
  },
  {
