...
export const CUTOVER_MAP: CutoverRoute[] = [
  ...
  },
  {
    ...
    desk: '/admin/desk/pulse',
    coverage: 'partial',
    note: "The funnel and categories carried over from the start. SEARCH INTEL and the DORMANT SEGMENT are now cards — both were live GETs nothing called. The zero-result half of search intel is deliberately given its own list rather than folded into the top terms: a popular term is a thing being found, and a term returning nothing is demand the storefront cannot serve, which is a stocking decision rather than a statistic. ⚠️ DORMANT IS THE ONE FIGURE ON PULSE THAT IGNORES THE PERIOD CHIPS — dormantSegment() counts against a fixed 14-day window in the service, so the card says so on its face; without that it reads as 'dormant in the last 7 days', a different and much smaller number. STILL LOST: THE TWO HEATMAPS. The endpoints exist and the wire helpers are written and tested (heatIndex keys on dow*24+hour, because a string concat merges dow 1/hour 12 with dow 11/hour 2, and DOW_LABELS is Sunday-first to match Postgres EXTRACT(DOW) — a Monday-first array shifts every row and reports the busiest trading day as the day before). What is missing is the grid component itself: components/desk/charts.tsx has no dow x hour primitive.",
  },
  {
