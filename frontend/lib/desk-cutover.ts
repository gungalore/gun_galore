...
export const CUTOVER_MAP: CutoverRoute[] = [
  ...
  },
  {
    ...
    coverage: 'partial',
    note: "Pulse reads the same AdminAnalyticsService methods over the SAME period vocabulary as the legacy page now. 🚨 THE PERIOD GAP WAS A TYPE, NOT A FEATURE: `type Period` stopped at 90d while the comment directly above it listed all five resolvePeriod() accepts, and every fetcher passes the value straight through — so 365d and all-time arrived by widening a union. The BUCKET control (day/week/month) is wired too, with a window-appropriate default, because 365 daily points on a sparkline is correct and unreadable. ⚠️ AND THE REFETCH DEPENDENCY WENT IN WITH IT — the loader was keyed on period alone, so the bucket chips would have lit up and fetched nothing, leaving the chart at the old resolution under a control saying otherwise. STILL LOST: top makes and models (a whole table, no Desk equivalent), time to sale, and CSV export — the last of which is not a pure frontend job, as no analytics CSV endpoint exists.",
  },
  {
