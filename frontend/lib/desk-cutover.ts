...
export const CUTOVER_MAP: CutoverRoute[] = [
  ...
  },
  {
    ...
    desk: '/admin/desk/site',
    coverage: 'partial',
    note: 'CORRECTED FROM replaced ON REVIEW. The low-balance flag it advertised did not exist: the Desk type invented vendor, currency, checkedAt and belowThreshold, none of which /admin/credits/snapshot sends, so every row rendered a blank name keyed on undefined and the warn tag could never fire. On a board whose own history is VerifyNow reaching 28 credits with no warning anywhere, a warning that cannot go off is worse than none. Fixed: the type is the wire shape, rows are keyed and named on service, and the flag is computed against /admin/credits/thresholds — for vendors whose pair reads as a floor. Anthropic encodes a daily SPEND CEILING in the same two columns and is left unflagged rather than flagged backwards, which is what the legacy page does. Still lost: editing a threshold, the non-billing test probe, and the per-vendor history chart.',
  },
  {
    legacy: '/admin/trust-safety',
    desk: '/admin/desk/site',
    coverage: 'partial',
    note: 'All five feeds are one Site section, segmented rather than five stacked tables: repeat offenders, reported Q and A, reported listings, reported sellers, and the contact-detail rejection log. Two deliberate differences, both improvements. The email address the legacy page printed under every username on all five tables is gone, per the privacy rule — username identifies the row and the person is worked on from People. And the intercepted text, which is usually a phone number or an address, is folded and opens one row at a time on a press instead of running down a column. CORRECTED FROM replaced ON REVIEW: this was marked replaced while its own note admitted the rows do not click through, which is the definition of partial. It is not cosmetic. Every legacy row was a link — a repeat offender, a reported seller and a rejecting user to /admin/users/[id], a reported listing and a reported question to /admin/listings/[id] — so the page was a queue you worked FROM. The Desk section is a list you read. The member half is one line to fix, because the Member drawer opens off any People row. The listing half is not: the Listing drawer only opens from a PENDING_REVIEW card, so a reported LIVE listing cannot be opened anywhere on the Desk and cannot be taken down from it. Close this to replaced when a reported row can open both.',
  },
  {
  ...
  },
  {
    ...
    desk: '/admin/desk/site',
    coverage: 'partial',
    note: "ALL THREE WRITES ARE WIRED (components/desk/admins-drawer.tsx): create, change-role and switch-off, each calling the SuperadminGuard-protected endpoint the legacy page used. This was the worst thing the cutover cost — the roster listed administrators and carried no control on any row, so removing a compromised one meant a database write. It does not any more. WHY IT IS STILL partial, AND IT IS NOT THE UI: MONITORING_ADMIN is documented as the read-only tier and is NOT ENFORCED. SuperadminGuard sits on exactly the three admin-management routes; every other admin endpoint takes any logged-in admin, so that role can today release a payout, refund a buyer, approve a listing and ban a member. The schema says so itself — \"(Currently not enforced on individual endpoints yet — that gate work is tracked separately.)\" The drawer therefore refuses to call it read-only and warns in the picker that it grants what full access grants; lib/desk-admins.spec.ts fails if anyone tidies that warning away. Close this to replaced when the role guard lands and the copy can honestly change — not before. Lockout is already unreachable: AdminService blocks changing your own role and deactivating yourself, so the sole Full admin cannot strand the account that would undo it.",
  },
  {
