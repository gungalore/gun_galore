...
export const CUTOVER_MAP: CutoverRoute[] = [
  ...
  },
  {
    ...
    desk: '/admin/desk/ledger',
    coverage: 'replaced',
    note: "The run preview was already replaced — segments, totals, the hold and lift levers, and a confirm that restates the sales, the sellers and what is held back. 🚨 DISBURSEMENT IS NOW WIRED, AND IT WAS THE ONE CALL THE DESK NEVER MADE. POST /admin/manual-payments/run-payouts existed with no caller anywhere in the frontend, and the confirm dialog on the Ledger closed itself and reported \"not wired from the Desk yet\", pointing at a handover note. So the board could show an operator exactly what was owed, to whom, and why a row was held back — and could not pay any of it. ⚠️ IT STAYS GATED ON PAYMENTS_LIVE, which is off. The service throws in words rather than returning a shape, and the board renders the gated variant instead of firing and then explaining: on a money surface, a control that says why it cannot act beats one that acts and apologises. ⚠️ EXACTLY-ONCE IS THE SERVER’S, via paidOutAt — pressing twice does not pay twice, but it does start a second Peach batch, so the button blocks re-entry while one is in flight. 🚨 AND THE RESULT SAYS ACCEPTED, NEVER PAID. Peach accepts a batch and settles it asynchronously; the payout webhook reconciles each one. \"12 sellers paid\" is a claim the platform cannot make at that moment, and it is exactly the sentence an operator would repeat to a seller. Pinned by test and mutation-verified. Two sibling endpoints on this controller are still uncalled and are worth someone’s attention: GET zoho-failed (every entity whose latest Books sync failed, in one list) and GET held-funds (the client-money position). The per-order Zoho retry now exists in the Order drawer, so the radar would make it findable without opening each sale.",
  },
  {
