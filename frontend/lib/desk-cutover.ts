 ...
 * WHY. The legacy panel is 32 pages. The Desk is four surfaces. Roughly
...
export const CUTOVER_MAP: CutoverRoute[] = [
  ...
  },
  {
    ...
    desk: null,
    coverage: 'none',
    note: 'CORRECTED FROM replaced TO none WHEN PULSE WAS DELETED IN PHASE 8, AND THIS IS A REAL LOSS RATHER THAN A NOTE CATCHING UP. The Desk has no analytics surface at all any more: no period chips, no daily-sales line, no listing-type split, no funnel, no top categories, no makes-and-models, no time-to-sale, no search intel, no dormant segment, no heatmaps. Fifteen live GETs on AdminAnalyticsService now have no caller in frontend/, and the sixteenth is the sharp one — GET /admin/analytics/export.csv was built for Pulse and was, in the words of the entry this replaces, the one thing on this whole map that genuinely needed building rather than connecting. It is an endpoint with no door. Nothing was deleted server-side, so restoring the board is wiring rather than building; what is gone is the wiring. ⚠️ THREE ENTRIES ON THIS MAP JUST WENT FROM replaced TO none, so the cutover can no longer be armed on analytics grounds — which is the correct state to be in, not a regression to paper over. THE LEGACY PAGE IS ALSO ALREADY DELETED, so this is not a live capability waiting to be redirected: it is a capability that no longer exists on either side. ⚠️ AND THIS ENTRY IS DELIBERATELY WRITTEN IN SINGLE QUOTES. scripts/desk-cutover.cjs parses the map with a regex that demands a single-quoted note, so the double-quoted entries are invisible to it; an entry whose coverage just regressed is written in the form the report can actually see.',
  },
  {
    legacy: '/admin/analytics/insights',
    desk: null,
    coverage: 'none',
    note: 'CORRECTED FROM replaced TO none WHEN PULSE WAS DELETED IN PHASE 8. What is gone with the board: the sign-up-to-first-sale funnel, top categories by revenue, search intel including the zero-result list that showed demand the storefront cannot serve, the dormant segment, and both dow x hour heatmaps. The heatmaps are the specific casualty worth naming, because they were the only item on the insights list that needed WRITING rather than calling — charts.tsx had no grid primitive and one was built for them. That primitive (Heatmap, HeatmapCell) is deleted with this entry; the five chart primitives beside it survive because app/admin/desk-kit renders them. The endpoints behind every one of these are untouched and still served, so the cost of restoring is wiring, not building.',
  },
  {
    legacy: '/admin/analytics/health',
    desk: null,
    coverage: 'none',
    note: 'CORRECTED FROM replaced TO none WHEN PULSE WAS DELETED IN PHASE 8. The three operational blocks go with it: KYC drop-off, the paid-to-dispatch distribution, and refund-risk sellers with the percentage-point distance above the marketplace baseline that is the entire reason a seller appears on that list. ⚠️ DO NOT CONFUSE THIS WITH /admin/health, WHICH IS A DIFFERENT ROUTE AND IS STILL replaced ON /admin/desk/health. This one was never the gates-and-channels page, and the name has already misled one pass of this map. Nothing here moved to Health; it is simply not rendered anywhere.',
  },
  {
    legacy: '/admin/health',
    desk: '/admin/desk/health',
    coverage: 'replaced',
    note: 'Services, crons and queue depths are a Health section, re-probing every 60 seconds with the age of the last sweep on screen, and a failed sweep leaves the previous reading up rather than blanking it. Services sort worst-first; the crons show the ones needing a look with the rest one press away, and the count comes off the response rather than being written down here — this note said 28 when cronStatuses defines 27, and it will move again the next time a job is added or a feature is deleted. Queue cards link to the Desk destination that reproduces the count where one exists — the HELD-past-dispatch-SLA card does not link, because the order book it points at is not built. SERVER VITALS ARE NOT HERE AND ARE NOT FAKED: CPU, memory and disk still need the Warden daemon, and the vitals card still says unknown rather than drawing a gauge with no source behind it. DESTINATION MOVED: this used to say /admin/desk/site. That board split in two, and the half an operator lands on for a health question is /admin/desk/health; the Warden conversation, the approval queue and the daemon run log went to /admin/desk/agent, which is a sixth tab. /admin/desk/site is now a 307 route handler onto Health and keeps its query string, so a bookmark still opens the drawer it named.',
  },
  {
    legacy: '/admin/settings',
    desk: '/admin/desk/health',
    coverage: 'partial',
    note: 'CORRECTED FROM replaced ON 2026-09-11, AND THE CORRECTION IS A REAL LOSS RATHER THAN A NOTE CATCHING UP. The settings panel was dropped from the board when Site split into Agent and Health, by instruction, so ops_alert_phone, ops_alert_types, ops_alert_quiet_hours and whatsapp_enabled are no longer editable from a browser at all. They were the only four flags that did not need a deploy, and whatsapp_enabled is the one control that exists to silence a channel in a hurry. Nothing else changed: PATCH /admin/settings still validates, still demands fifteen characters of reason on the danger flag, still writes an audit row, and GET /admin/warden/settings still shapes and masks the four rows. Both are simply uncalled now, along with fetchDeskSettings, fetchWardenSettings, updateSetting and settingReasonMin in lib/desk-site.ts. THE ONE CONTROL THAT SURVIVED IS THE CREDIT FLOOR EDITOR, because a floor tracks supplier pricing rather than a code constant, and PUT /admin/credits/thresholds is not a settings write. 🚨 THIS ENTRY IS DELIBERATELY WRITTEN IN SINGLE QUOTES. scripts/desk-cutover.cjs parses the map with a regex that demands a single-quoted note, so the twenty entries written with double quotes are invisible to it — it reports nine of twenty-nine routes and says READY. This entry is the one whose coverage just regressed, so it is written in the form the gate can actually see; the regex itself lives in scripts/ and belongs to whoever owns that directory. WHAT WOULD CLOSE THIS: a four-row editor anywhere on the Desk, or an operator decision that a deploy is an acceptable cost for all four.'
  },
  {
    legacy: '/admin/audit',
    desk: '/admin/desk/health',
    coverage: 'replaced',
    note: "The audit trail is a drawer on Health — read-only, newest first, the actor and the reason on every row. REACH BACKWARDS IS NOW CLOSED, which was the whole of this entry: it read the newest fifty, cached them forever and stopped, so 'who released that payout in July' was unanswerable on the one surface built to answer it. `offset` was accepted by the server the entire time and never sent, and `resourceType` was a declared parameter of fetchAudit that no caller passed. Both are now wired, with a chip row, Newer/Older paging and the range and total on screen. ⚠️ THE CACHE GUARD WAS PART OF THE BUG: the parent fetched behind `if (!audit)`, which is right for a list that never changes and wrong the moment a filter exists — the second chip press would have redrawn the first chip's rows. State moved into the drawer, keyed on the filter. ⚠️ THE CHIPS ARE TRANSCRIBED FROM THE resourceType LITERALS ACTUALLY WRITTEN IN backend/src, not guessed — the first draft was guessed and offered Complaint and AdminUser, neither of which any writer emits, so both would have returned an empty log reading as 'nothing has ever happened to a complaint'. Everything is always the default, so a type with no chip stays reachable. adminUserId filtering exists server-side and is deliberately not offered: nothing on the Desk resolves an admin identity to pick from.",
  },
  {
    legacy: '/admin/alerts',
    desk: '/admin/desk/health',
  ...
  },
  {
    ...
    desk: null,
    coverage: 'retired',
    note: 'STILL RETIRED, BUT THE COMPENSATION NAMED IN THIS NOTE IS GONE AND THE NOTE HAD TO SAY SO. Editing the tree goes to code because isFirearm and requiresLicence decide what sits behind the members-only gate and a web form leaves no diff to review. Going with them, and worth agreeing to on purpose: isActive, which hides a category from the Sell form and the browse filters and is a same-day merchandising lever; sortOrder; availableSecondhand and availableNewStore; crossSellEligible; and the crossSellTo relationship map. ⚠️ THE UNMET CROSS-SELL DEMAND BLOCK WAS THIS ENTRY’S ONE COMPENSATION AND IT WENT WITH PULSE IN PHASE 8. It was a read-only running tally of the pairings buyers asked for, and it was the reason an operator could at least SEE demand they could no longer wire without a deploy. There is now no view of it anywhere on the Desk. Coverage is held at retired rather than moved to none because the DECISION to retire the editor stands and was never about the tally — but whoever revisits this should know the reading half is gone too, and that is a second, smaller decision nobody has taken. The tree is still readable via GET /admin/categories, and the demand figures are still served.',
  },
  {
